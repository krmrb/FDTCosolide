<template>
   <main class="main-content bg-white flex-column flex-fill p-3 p-sm-5 pt-sm-3">
      <div class="container text-start">
         <div class="d-flex align-items-center gap-3 mb-4">
            <button class="btn btn-link text-decoration-none p-0" @click="retourArriere">
            ← Retour
            </button>
            <h3 class="mb-0 text-primary">Horaire</h3>
         </div>
               ...
               <ul class="dropdown-menu p-3" style="min-width: 250px;">
                  <li>
                     <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="Absences" id="checkAbsences" checked />
                        <label class="form-check-label" for="checkAbsences">Absences</label>
                     </div>
                     <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="Paiements" id="checkPaiements" checked />
                        <label class="form-check-label" for="checkPaiements">Paiements</label>
                     </div>
                  </li>
            ...
            <div class="modal-dialog modal-lg">
               <div class="modal-content">
                  <div class="modal-header">
                     <h5 class="modal-title">Demande d'absence</h5>
         ...
         <div v-if="showFormulaireAbsence" class="modal-backdrop fade show"></div>
         <div v-if="showFormulaireAbsence" class="modal d-block" tabindex="-1">
            <div class="modal-dialog modal-lg">
               <div class="modal-content">
                  <div>
                     <DemandeModal 
                        :demande="null" 
                        affichage="popup"
                        @fermer="showFormulaireAbsence = false"
                        />
                  </div>
               </div>
            </div>
         ...
         <!-- Formulaire Paiement -->
         <div v-if="showFormulairePaiement" class="modal-backdrop fade show"></div>
         <div v-if="showFormulairePaiement" class="modal d-block" tabindex="-1">
            <div class="modal-dialog modal-lg">
               <div class="modal-content">
                  <div class="modal-header border-0">
                     <button type="button" class="btn-close" @click="showFormulairePaiement = false" aria-label="Fermer"></button>
                  </div>
                  <div class="modal-body">
                     <DemandePaiementModal 
                        :demande="null" 
                        @fermer="showFormulairePaiement = false"
                        />
                  </div>
               </div>
            </div>
...
import FormulaireAbsence from '@/components/absence/FormulaireAbsence.vue'
...
const handleOpenAbsenceForm = () => {
  showFormulaireAbsence.value = true
}

const showFormulaireAbsence = ref(false)
const showFormulairePaiement = ref(false)
...
onMounted(() => {
  ...
  window.addEventListener('openAbsenceForm', handleOpenAbsenceForm)
  window.addEventListener('openPaiementForm', handleOpenPaiementForm) // ← CETTE LIGNE DOIT EXISTER
})

onUnmounted(() => {
  window.removeEventListener('openAbsenceForm', handleOpenAbsenceForm)
  window.removeEventListener('openPaiementForm', handleOpenPaiementForm) 
})

const handleOpenPaiementForm = () => {
  showFormulairePaiement.value = true
}
...
function modifierElement() {
  ...
  // Méthode robuste pour détecter le type
  const eventType = event.extendedProps?.type || event.type || event.title
  ...
  if (eventType.includes('Absence')) {
    ...
    console.warn('Type d\'événement non reconnu:', eventType)
  }
