        ...
        <table class="table-demandes">
          <thead class="">
            <tr>
              ...
              <th class="text-middle col-date-de"><i class="bi bi-calendar-event me-1 text-primary"></i> Date d'absence</th>
          ...
          <tbody>
            <tr v-for="item in demandesFiltrees" :key="item.id" @click="selectionnerDemande(item)"
              ...
              <td :title="item.noteApprobation">
                {{ item.noteApprobation.length > 25 ? item.noteApprobation.slice(0, 25) + '...' : item.noteApprobation
                }}
                <i v-if="item.noteApprobation.length > 25" class="bi bi-chat-left-text text-muted ms-1"></i>
              </td>
