  ...
  <div class="px-4">
    ...
    <div class="bg-light p-4">
      ...
      <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-4">
          <input type="date" id="date_deb" class="form-control" v-model="form.dateDebut" />
        </div>
      </div>
      <!-- Date fin-->
      <div class="row mb-3 align-items-center">
      ...
      <div class="mt-4 text-end d-flex justify-content-end gap-2 flex-wrap">
        ...
        <!-- Bouton conditionnel : Enregistrer ou Annuler la demande -->
        <button v-if="estEnCreation"
                type="button"
                class="btn btn-link"
                @click="enregistrerEtFermer">
          Enregistrer
        </button>
        ...
        <button v-else
                ...
                v-bind:disabled="!formulaireValide"
                @click="annuler">
          Annuler la demande
        </button>

        <!-- Bouton Soumettre ou Modifier -->
        <button type="button"
                class="btn btn-primary"
                v-bind:disabled="!formulaireValide"
                @click="soumettreEtFermer">
          {{ modeSoumission }}
        </button>
      </div>
    </div> 
...
<script setup lang="ts">
  ...
  // Formulaire
const form = ref<Demande>(props.evenement ? { ...props.evenement } : initialiserNouvelleDemande())
...
const labelMotif = computed(() => {
  if (!form.value.motif) return ''
  const motif = motifsAbsence.find(m => m.code === form.value.motif)
  return motif ? motif.label : form.value.motif
})
...
function annuler() {
  ...
  console.log('Demande annulée :', form.value)
  form.value.etatValidation = ValidationStatut.RetireMaDemande // ou 'Annulé'
  emit('demandeModifiee', form.value)
  emit('fermer')
}
