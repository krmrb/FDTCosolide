<template>
  <main class="flex-fill d-flex flex-column min-vh-100">
    <div class="main-content bg-white flex-column flex-fill">
      <table class="table table-hover">
        ...
        <tbody>
          <tr v-for="demande in demandesFiltrees" :key="demande.id"
              @click="$emit('ligneCliquee', demande)"
            ...
            <td align="center">
              <span style="white-space: nowrap">
              <button v-if="demande.etat !== 'Brouillon'" class="btn btn-sm" @click.stop="$emit('voirDetails', demande)">
                <i class="bi-eye text-secondary" title="Voir les détails"></i>
              </button>
              <button v-if="demande.etat === 'Brouillon'" class="btn btn-sm" @click.stop="$emit('modifier', demande)">
                <i class="bi-pencil-square text-secondary" title="Modifier"></i>
              </button>
              <button v-if="['Soumis', 'Brouillon'].includes(demande.etat)" class="btn btn-sm" @click.stop="$emit('retirer', demande)">🗑️</button>
              </span>
            </td>
          </tr>
        </tbody>
